...
def get_defeaters_brute_force(bn, ev_vars, hyp_vars, depth=np.inf):
...
def get_defeaters_pgm(bn, ev_vars, hyp_vars, depth=np.inf, evaluate_singletons=False):
    # Check which are the supplementary variables
    variables = bn.names()
    supp_vars = []
    for var in variables:
    ...
    # supp_vars_og contains all supplementary nodes
